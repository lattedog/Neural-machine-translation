...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, input_sizes = pad_packed_sequence(output, batch_first=True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, 
        ...
        for y in torch.split(Y, 1, dim = 0):
            ...
            Ybar_t = torch.cat( (torch.squeeze(y, dim = 0), o_prev), dim = 1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        h_dec_t = torch.unsqueeze(dec_hidden, 1)
        
        e_t = torch.squeeze(torch.bmm(h_dec_t, torch.transpose(enc_hiddens_proj, 1, 2) ) , 1)    # shape = (b, src_len)
